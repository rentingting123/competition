...
let userRandom = (config, fileName) => {
  ...
  let date = new Date()
  if(!!config.temp) return `temp/${User.nickname ? User.nickname+'/' : ''}${fileName}`
  //文件名外面的文件夹命

  const folderName = folder ? folder + '/' : '';
  //2021.7.16 刘军(网络技术挑战)要求拿掉作品的用户名
  const nickname = ''//User.nickname ? User.nickname + '/' : '';
  const _fileName = config.noHash ? fileName : `${util.randomChar(RANDOM_CHAR_LENGTH)}_${date.getTime()}`;
  return `${folderName}${nickname}${_fileName}`
}
...
var _getOss = async (callback) => {
    let token = await network.getOssKey()
...
function _upload(file, callback, progress, config={},checkUploadEnable = () => true) {
  _getOss(async (err, ossUpload) => {
    ...
    const gId =await localStorage.getItem("competitionId") || await localStorage.getItem("compId")//竞赛ID
    var fileName = file.name || ''
       , fileSuffix = fileName.match(/.*(\.\w+)/) || ''
       , fileId =`${gId ? gId : 'unknow'}/${FILE_PATH.replace('{RANDOM_CHAR}', userRandom(config,fileName))}` 
       , fileKey
