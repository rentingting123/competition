...
<div>
    <el-table :empty-text="$t('yaoqing.noTeam')" :data="teams" :header-cell-style="{background:'#f3f3f3'}" class="table" border style="">
        ...
        <!-- <el-table-column prop="inviteStatus" v-if="current !== 3" :label="$t('yaoqing.status')" >
            <template slot-scope="scope">
                <span v-if="scope.row.inviteStatus == 0">待对方确认</span>
                <span v-else-if="scope.row.inviteStatus == 1">对方已同意</span>
                <span v-else>对方已拒绝</span>
            </template>
        </el-table-column> -->
         <el-table-column prop="applyStatus" :label="$t('yaoqing.status')" >
            <template slot-scope="scope">
                <span v-if="scope.row.applyStatus == 0">待确认</span>
                <span v-else-if="scope.row.applyStatus == 2">已拒绝</span>
                <span v-else-if="scope.row.applyStatus == 1">已同意</span>
                <span v-else>暂无</span>
            </template>
        </el-table-column>
...
export default {
    ...
    methods:{
       ...
       async tongyi(){
        ...
        Group({data:{inviteRecordId:this.inviteRecordId,status:this.agreen}})   
         let res = await network.agreeJoin
