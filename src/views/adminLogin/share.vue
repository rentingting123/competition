<template>
    <el-row class="adminlogin">
       <el-col :span="24">
           <div class="middle">
                <div class="header">
                    <img class="logo" src="https://web.moocollege.com/mooc/hzq/competition/share/%E6%91%A9%E8%AF%BE%E4%BA%91.png" >
                    <img class="logo1" src="https://web.moocollege.com/mooc/hzq/competition/share/%E5%9B%BE%E7%89%87.gif" >
                </div>
               <div class="team" v-if="teamlist">
                   <h2>
                       <span>{{teamlist.groupName}}</span>
                       <span v-if="teamlist.awardName"><i class="prize"></i>{{teamlist.awardName}}</span>
                    </h2>
                   <p>团队成员</p>
                   <ul>
                ...
                <div class="workdetail" v-if="works">
                    <h2>
                        {{works.title}}
                        <span v-if="works.titleEn"> - {{works.titleEn}}</span>
                        <p></p>
                    </h2>
                    <div>
...
export default {
    ...
    created(){
        ...
        this.mdPoolId = this.$route.query.mdPoolId;
        if(!this.mdPoolId) return this.$message.warning('未获取到对应的作品')
        this.getTeam();
    },
    methods:{
        async getTeam(){
            let teamlist = await network.getTeamListNoPrmission({data:{ mdPoolId: this.mdPoolId }});
            if (teamlist == "error") return;
            this.teamlist = teamlist
            this.getworksDetail();
        },
        async getworksDetail() {
            let data = await network.getWorkRuleByCompetitionIdNoPrmission({data:{ mdPoolId: this.mdPoolId }});
